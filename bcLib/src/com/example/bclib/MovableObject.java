...
public class MovableObject extends MapObject {
	...
	public void setNewAngle() {
		if (this.stop == 0 && this.angle != Math.PI / 2) {
			if (this.angle > Math.PI / 2 && this.increment > 0
					|| this.angle < Math.PI / 2 && this.increment < 0) {
				this.increment *= -1;
			}
		...
		} else if (this.angle + this.increment <= ((Math.PI / 2) + this.stop)
				&& this.angle + this.increment >= ((Math.PI / 2) - this.stop)
				&& this.stop != 0) {
			this.angle += this.increment;
		}
	}
